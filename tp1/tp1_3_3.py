...
def letra_e():
    sql = f"""
            select prod.asin, produto.title, prod.avrrating, prod.total_reviews
            from (
                select revs.asin as asin, avg(revs.rating) as avrrating, count(revs.rating) as total_reviews
                from (
                    select *
                    from review r
                    where ( r.votes > 0 )
    ...
    sql = f"""
            ...
            from categoria,(
                ...
                from (
                    select *
                    from (
                        select asin, avg(rating) as avrrating, count(asin) as total_reviews
                        from (
                            select *
                            from review r
                            where ( r.votes > 0)
